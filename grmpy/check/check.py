...
def check_initialization_dict(dict_):
    ...
    if dict_["ESTIMATION"]["file"][-4:] not in [".pkl", ".txt", "dta"]:
        msg = (
            "The {} format specified in the Estimation section of the initialization file"
...
def check_init_file(dict_):
    ...
    for key_ in ["TREATED", "UNTREATED", "CHOICE"]:
        if len(set(dict_[key_]["order"])) != len(dict_[key_]["order"]):
            ...
            raise UserError(msg)
        if "params" not in dict_[key_].keys() and dict_["ESTIMATION"]["start"] == "init":
            msg = (
                "The missing of a pre-specified paramterization in the {} section does not "
                "correspond with the start value option of your initialization file. \n "
                "        We recommend to switch to the generation of automatic start values by "
                'changing the start flag in the ESTIMATION section from "init" to "auto".'.format(
                    key_
                )
            )
            raise UserError(msg)
...
def check_start_values(x0):
    if False in np.isfinite(x0):
        msg = (
            "The automatic start value generating process did not lead to finite start values "
