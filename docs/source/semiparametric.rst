...
Alternatively, ``grmpy`` can estimate the MTE via the semiparametric method of Local Instrumental Variables (LIV).

Assumptions
------------

Consider again the normal linear-in-parameters version of the generalized Roy model.
Both potential outcomes and the choice :math:`(Y_1, Y_0, D)` are a linear function of the individual's
observables :math:`(X, Z)` and random components :math:`(U_1, U_0, V)`.
